...
if (${CMAKE_SYSTEM_NAME} STREQUAL "Android")
	set(COMMON_ARGS -DCMAKE_VERBOSE_MAKEFILE=TRUE -DANDROID_DEPRECATED_HEADERS=ON -DCMAKE_CXX_STANDARD=${CMAKE_CXX_STANDARD} -DCMAKE_SYSTEM_NAME=${CMAKE_SYSTEM_NAME} -DCMAKE_SYSTEM_VERSION=${CMAKE_SYSTEM_VERSION} -DCMAKE_ANDROID_NDK_TOOLCHAIN_VERSION=${CMAKE_ANDROID_NDK_TOOLCHAIN_VERSION} -DCMAKE_ANDROID_STL_TYPE=${CMAKE_ANDROID_STL_TYPE} -DCMAKE_ANDROID_NDK=${CMAKE_ANDROID_NDK} -DCMAKE_ANDROID_ARCH_ABI=${CMAKE_ANDROID_ARCH_ABI} -DCMAKE_AR=${CMAKE_AR} -DCMAKE_MAKE_PROGRAM=${CMAKE_MAKE_PROGRAM} -DCMAKE_SYSTEM_VERSION=${CMAKE_SYSTEM_VERSION})
elseif(ANDROID)
    set(COMMON_ARGS -DANDROID_DEPRECATED_HEADERS=ON -DCMAKE_SYSTEM_NAME=${CMAKE_SYSTEM_NAME} -DCMAKE_SYSTEM_VERSION=${CMAKE_SYSTEM_VERSION} -DCMAKE_ANDROID_STL_TYPE=${CMAKE_ANDROID_STL_TYPE} -DCMAKE_ANDROID_NDK=${CMAKE_ANDROID_NDK} -DCMAKE_ANDROID_ARCH_ABI=${CMAKE_ANDROID_ARCH_ABI} -DANDROID_STL=${ANDROID_STL} -DCMAKE_AR=${CMAKE_AR} -DCMAKE_MAKE_PROGRAM=${CMAKE_MAKE_PROGRAM} -DANDROID_NDK=${ANDROID_NDK} -DCMAKE_TOOLCHAIN_FILE=${CMAKE_TOOLCHAIN_FILE} -DANDROID_NATIVE_API_LEVEL=${ANDROID_NATIVE_API_LEVEL} -DANDROID_ABI=${ANDROID_ABI})
endif()
...
if(APPLE)
    ...
    set(COMMON_ARGS -DCMAKE_SYSTEM_NAME=${CMAKE_SYSTEM_NAME} -DCMAKE_OSX_ARCHITECTURES=${APPLE_ARCHS} -DCMAKE_OSX_SYSROOT=${CMAKE_OSX_SYSROOT})
...
macro(expose_external_library STATIC_OR_SHARED)
    add_library(${LIBRARY_NAME} ${STATIC_OR_SHARED} IMPORTED GLOBAL)
...
function(expose_additional_external_library LIBRARY_NAME)
    ...
    set_external_library(${LIBRARY_NAME} FALSE "${LINK_DEPENDENCY}" "${EXTERNAL_DEPENDENCY}")
    expose_external_library(STATIC)
endfunction()

macro(set_external_library NAME IS_SHARED_LIB)
    set(LIBRARY_NAME ${NAME})
    if (MSVC)
        set(LIBRARY_PATH ${EXTERNAL_LIBRARIES_INSTALL_PATH}/lib/${LIBRARY_NAME}.lib)
    else(MSVC)
		if (${IS_SHARED_LIB})
		    set(LIBRARY_PATH ${EXTERNAL_LIBRARIES_INSTALL_PATH}/lib/lib${LIBRARY_NAME}.so)
		else()
    ...
    set(LINK_DEPENDENCY ${ARGV2})
    set(EXTERNAL_DEPENDENCY ${ARGV3})
endmacro()